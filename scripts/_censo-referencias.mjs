...
for (const [unidad, rel] of UNIDADES) {
  for (const p of mdxDe(rel)) {
    ...
    for (let i = 0; i < lineas.length; i++) {
      ...
      // Ya reformateado (una ficha por parrafo) si NO quedan « · » separadores.
      const fichas = sinRotulo.split(' · ').map((f) => f.trim()).filter(Boolean);
      bloques.push({ unidad, f: path.relative(contentDir, p).replace(/\\/g, '/'), linea: i + 1, rotulo, n: fichas.length, fichas, lineas: cuerpo.length });
      i += cuerpo.length - 1;
    }
  }
}
...
const multi = bloques.filter((b) => b.n > 1);
console.log(`\n  bloques con MAS DE UNA ficha en el mismo parrafo: ${multi.length}`);
console.log(`  el peor: ${multi.length ? multi.reduce((a, b) => (b.n > a.n ? b : a)).f + ' con ' + multi.reduce((a, b) => (b.n > a.n ? b : a)).n : '—'}`);
